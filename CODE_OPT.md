...
| `JIT_OPT_RTN_IC_2WAY` | 2-way polymorphic IC. Two `(ic_ret_pc, ic_ret_body)` slots per dyn-end entry; JIT emits two compares with a common hit path; dispatcher alternates fill slot on each miss. | Polymorphic 2-way alternation (nqueens calls Ptst from two GOSUB sites in close succession) goes from 100% miss to 100% hit. Monomorphic call sites still hit slot 0 on the first compare and pay no extra cost. |
| `JIT_OPT_LAZY_EXIT_PC` | Defer `movw r0, #next_pc` past the chain BGE in self-loop blocks; the BGE-taken path goes straight to body which doesn't read r0. | Saves one 4-byte `movw` per chained iter in tight self-loops (arith / memmix / countloop body blocks). |
